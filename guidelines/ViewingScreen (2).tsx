...
type GenStatus = 'generating' | 'polling' | 'done' | 'error';
...
export function ViewingScreen() {
  ...
  const storyId: string         = location.state?.storyId       || String(Date.now());

  const [genStatus, setGenStatus] = useState<GenStatus>('generating');
  ...
  useEffect(() => {
    if (!isPlaying || isFinished || videoUrl) return;
    const t = setInterval(() => {
  ...
  }, [isPlaying, isFinished, videoUrl, savedDrawings.length]);
  ...
  async function startGeneration() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`Server error: ${res.status} — is server.js running on port 3001?`);
    ...
    pollTimer.current = setInterval(async () => {
      try {
        ...
        if (job.status === 'done') {
          ...
          clearInterval(pollTimer.current!);
          setGenStatus('error');
          setErrorMsg(job.error || 'Veo returned an error.');
        }
      } catch (e: any) {
        clearInterval(pollTimer.current!);
        setGenStatus('error');
  ...
  const guideMessage = () => {
    if (genStatus === 'done')    return 'Your animation is ready — press play! 🎬';
    if (genStatus === 'error')   return 'Hmm, something went wrong. Try again!';
    if (genStatus === 'polling') return `Animating your story... ✨ (${pollCount * 8}s)`;
    return 'Sending your drawing to the animation studio... 🎨';
  };

  const curtainsOpen = (isPlaying && !isFinished) || !!videoUrl;
    ...
    <div className="size-full bg-white flex flex-col items-center justify-between py-2 px-4 overflow-hidden border-[24px] border-[#E63946]">
      <div className="w-full max-w-6xl flex justify-between items-center z-10">
        ...
        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
          ...
          className="bg-yellow-400 text-gray-800 px-6 py-3 rounded-full font-bold shadow-lg flex items-center gap-2"
        >
      ...
      <div className="z-10 -mt-2">
        <StoryGuide message={guideMessage()} isAnimating={true} />
      </div>

      <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 0.85, opacity: 1 }}
        transition={{ duration: 0.5, type: 'spring' }} className="relative -mt-10"
      >
        <div className="relative bg-gradient-to-br from-gray-900 to-gray-800 p-6 rounded-3xl shadow-[0_0_40px_rgba(0,0,0,0.5)] -mt-4 border-[6px] border-red-950">
          ...
          <div className="relative bg-black rounded-2xl overflow-hidden shadow-inner" style={{ width: '640px', height: '360px' }}>
            <motion.div className="absolute top-0 left-0 w-1/2 h-full z-10 bg-gradient-to-r from-red-900 via-red-700 to-red-800 border-r-4 border-red-950 pointer-events-none"
              animate={{ x: curtainsOpen ? '-100%' : '0%' }} transition={{ duration: 1.5, ease: 'easeInOut' }}
            />
            ...
            {videoUrl ? (
              <motion.div key="veo" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 z-20">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center z-20 gap-4 bg-gray-900 text-white p-6 text-center">
                ...
                <p className="text-sm text-gray-300 max-w-xs">{errorMsg}</p>
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                  onClick={startGeneration}
                  className="bg-yellow-400 text-gray-900 px-6 py-3 rounded-full font-bold mt-2"
                  style={{ fontFamily: 'Comic Sans MS, cursive' }}
                >Try Again</motion.button>
              </div>
            ) : !isPlaying && !isFinished ? (
              <motion.div className="absolute inset-0 flex flex-col items-center justify-center z-20 gap-3 bg-black/70 cursor-pointer"
                onClick={() => { setIsPlaying(true); setCurrentSlide(0); setIsFinished(false); }}
              >
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                  className="flex flex-col items-center gap-4 bg-black/40 px-10 py-8 rounded-3xl backdrop-blur-sm border border-yellow-500/30"
                >
                  ...
                  <p className="text-yellow-300 text-sm" style={{ fontFamily: 'Comic Sans MS, cursive' }}>
                    Veo animation generating in background ✨
                  </p>
                </motion.div>
              </motion.div>
            ) : isFinished ? (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 1.5 }}
                className="absolute inset-0 flex flex-col items-center justify-center z-20 gap-6 bg-black/50 backdrop-blur-sm"
              >
                ...
                <div className="flex gap-6">
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                    ...
                    style={{ fontFamily: 'Comic Sans MS, cursive' }}
                  ><RotateCcw className="w-6 h-6" /> Watch Again</motion.button>
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                    ...
                    style={{ fontFamily: 'Comic Sans MS, cursive' }}
                  ><PlayCircle className="w-6 h-6" /> Create New!</motion.button>
                </div>
                {genStatus === 'polling' && (
                  <p className="text-yellow-300 text-sm" style={{ fontFamily: 'Comic Sans MS, cursive' }}>
                    🎬 Veo still animating ({pollCount * 8}s) — video will appear when ready!
                  </p>
                )}
              </motion.div>
            ) : (
